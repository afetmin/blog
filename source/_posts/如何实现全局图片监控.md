...
tags: [监控]
description: `为什么要做图片的全局监控？图片过大占用 CDN 资源，拖慢加载速度，体验不好。应该怎么做？主要拿到图片的原始宽高和图片显示的实际宽高，超出一定比例，图片大小超过一定阈值（比如 1M），基本可以判断图片不太符合规范，上报该数据。上报的数据可以是图片的 DOM 路径，用于定位排查。`
---
